...
(function ($) {
  ...
  // Auto apply conversion of number fields with class 'rating' into rating-fields
  $(function () {
    if ($('input.cur_rating[type=number]').length > 0) {
      $('input.cur_rating[type=number]').cur_rating();
    }
  });
...
function lendBook()
{
 ...
 function submitBook()
 {
  ...
  function rateByUser()
  {
    var rating=document.getElementById("ratingval").value;
    var name=document.getElementById("nameDoc").value;
     ...
     if(rating && name)
     {
         ...
         success: function (response)
         {
              if(response!="OK")
              {
                alert("Αποτυχία καταχώρησης βαθμολογίας!");
                return false;
              }
              else
              {
        ...
        alert("Σφάλμα κατά την καταχώρηση βαθμολογίας!");
        return false;
      }
